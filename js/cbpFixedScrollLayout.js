 ...
 * http://www.codrops.com
 * 
 * Edited (normalized) by Konstantin Polyakov
 */
var cbpFixedScrollLayout = (function() {
	...
	function init(options={}) {
        ...
        config.$sections = options['sections'] || config.$sections;
        config.$navlinks = options['navlinks'] || config.$navlinks;

		// click on a navigation link: the body is scrolled to the position of the respective section
		config.$navlinks.on( 'click', function() {
			//scrollAnim( config.$sections.eq( $( this.parentElement ).index() ).offset().top );
            if (this.origin == location.origin) scrollAnim($( this.hash ).offset().top);
			else return true;
			return false;
		} );
